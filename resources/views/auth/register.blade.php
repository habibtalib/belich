    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="container mx-auto h-full flex justify-center items-center">
            <div class="w-1/3 max-w-md">
                {{-- Title --}}
                <h1 class="text-2xl text-teal-700 font-semibold mb-6 text-center">@lang('belich::authorization.register.create')</h1>
                <div class="p-8 bg-white mb-6 rounded-lg shadow-lg">
                    {{-- Name --}}
                    <div class="mb-6">
                        <label class="font-bold text-grey-700 block mb-2">@lang('belich::authorization.register.name')</label>
                        <input type="text" name="name" class="block appearance-none w-full bg-white border border-grey-400 hover:border-grey-500 px-2 py-2 rounded shadow" value="{{ old('name') }}" placeholder="{{ trans('belich::authorization.placeholder.name') }}" autofocus>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        {{-- Login --}}
                        <a class="no-underline inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-600 float-right" href="{{ route('login') }}">
